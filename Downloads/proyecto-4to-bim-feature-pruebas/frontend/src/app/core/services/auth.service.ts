...
export class AuthService {
  ...
  login(credentials: LoginRequest): Observable<LoginResponse> {
    const body: LoginRequest = {
      usuario_correo: credentials.usuario_correo.trim().toLowerCase(),
      usuario_password: credentials.usuario_password
    };

    return this.http
      .post<LoginResponse>(`${environment.apiUrl}/usuarios/login`, body)
      .pipe(
        tap((response) => {
  ...
  registerCandidate(data: RegisterRequest): Observable<RegisterResponse> {
    ...
    return this.http.post<RegisterResponse>(`${environment.apiUrl}/usuarios`, body);
  }

  registerEmpresa(data: RegisterEmpresaRequest): Observable<RegisterEmpresaResponse> {
    const body: RegisterEmpresaRequest = {
      usuario_nombre: data.usuario_nombre.trim(),
      usuario_apellido: data.usuario_apellido.trim(),
      usuario_correo: data.usuario_correo.trim().toLowerCase(),
      usuario_password: data.usuario_password,
      ...
      empresa_descripcion: data.empresa_descripcion?.trim() || undefined,
      empresa_correo: data.empresa_correo.trim().toLowerCase(),
      empresa_telefono: data.empresa_telefono?.trim() || undefined,
      empresa_nit: data.empresa_nit?.trim() || undefined,
      empresa_direccion: data.empresa_direccion?.trim() || undefined
    };

    return this.http.post<RegisterEmpresaResponse>(`${environment.apiUrl}/usuarios/empresa`, body);
  }

...
import { Usuario, RolUsuario, ActualizarUsuarioAdminRequest, CambiarPasswordAdminRequest } from '../../../../core/models/auth.model';
...
export class AdminUsersForm implements OnInit {
  ...
  readonly usuarioForm = this.fb.nonNullable.group({
    ...
    usuario_rol: ['Candidato' as RolUsuario, Validators.required],
    estado: [true, Validators.required]
  });

  readonly passwordForm = this.fb.nonNullable.group({
    usuario_password_nueva: ['', [Validators.required, Validators.minLength(6)]],
  ...
  get usuariosFiltrados(): Usuario[] {
    ...
    return this.usuarios.filter(usuario => {
      const coincideBusqueda =
        !texto ||
        `${usuario.usuario_nombre} ${usuario.usuario_apellido}`.toLowerCase().includes(texto) ||
        usuario.usuario_correo.toLowerCase().includes(texto) ||
        (usuario.usuario_dpi ?? '').toLowerCase().includes(texto);

      const coincideRol =
        !this.filtroRol ||
        usuario.usuario_rol === this.filtroRol;

      const coincideEstado =
        !this.filtroEstado ||
        String(usuario.estado) === this.filtroEstado;
  ...
  editarUsuario(usuario: Usuario): void {
    ...
    this.usuarioForm.reset({
      ...
      usuario_profesion: usuario.usuario_profesion ?? '',
      usuario_rol: usuario.usuario_rol,
      estado: usuario.estado ?? true
    });

    const miId = this.authService.currentUsuario()?.usuario_id;

    if (usuario.usuario_id === miId) {
      this.usuarioForm.controls.usuario_rol.disable();
      this.usuarioForm.controls.estado.disable();
    } else {
      this.usuarioForm.controls.usuario_rol.enable();
      this.usuarioForm.controls.estado.enable();
    }
  ...
  guardarUsuario(): void {
    ...
    const data: ActualizarUsuarioAdminRequest = {
      ...
      usuario_dpi: value.usuario_dpi.trim(),
    ...
    this.usuarioService.actualizarUsuario(this.usuarioSeleccionado.usuario_id, data)
      .pipe(
        finalize(() => {
      ...
      .subscribe({
        next: response => {
          this.mensaje = response.message || 'Usuario actualizado correctamente.';

          this.usuarios = this.usuarios.map(usuario =>
            usuario.usuario_id === response.usuario.usuario_id
              ? response.usuario
              : usuario
          );
          ...
          if (response.usuario.usuario_id === miId) {
            this.authService.updateCurrentUsuario(response.usuario);
          }
  ...
  cambiarEstado(usuario: Usuario): void {
    ...
    if (usuario.usuario_id === miId) {
      this.error = 'No puedes desactivar tu propia cuenta desde esta pantalla.';
      this.cdr.detectChanges();
      return;
    }

    const nuevoEstado = usuario.estado !== true;
    ...
    this.usuarioService.cambiarEstado(usuario.usuario_id, nuevoEstado)
      .pipe(
        finalize(() => {
      ...
      .subscribe({
        next: response => {
          this.usuarios = this.usuarios.map(item =>
            item.usuario_id === response.usuario.usuario_id
              ? response.usuario
              : item
          );

          if (this.usuarioSeleccionado?.usuario_id === response.usuario.usuario_id) {
            this.usuarioSeleccionado = response.usuario;
            this.usuarioForm.patchValue({
              estado: response.usuario.estado ?? false
            });
          }
  ...
  mostrarCambiarPassword(usuario: Usuario): void {
    this.usuarioSeleccionado = usuario;
    this.mostrarFormulario = true;
  ...
  guardarPassword(): void {
    ...
    if (value.usuario_password_nueva !== value.confirmar_password) {
      this.error = 'Las contraseñas no coinciden.';
      return;
    }

    const data: CambiarPasswordAdminRequest = {
      usuario_password_nueva: value.usuario_password_nueva
    };

    this.cambiandoPassword = true;

    this.usuarioService.cambiarPasswordAdmin(this.usuarioSeleccionado.usuario_id, data)
      .pipe(
        finalize(() => {
  ...
  eliminarUsuario(usuario: Usuario): void {
    ...
    if (usuario.usuario_id === miId) {
      this.error = 'No puedes eliminar tu propia cuenta desde esta pantalla.';
      this.cdr.detectChanges();
      return;
    }

    const nombre = `${usuario.usuario_nombre} ${usuario.usuario_apellido}`;
    ...
    this.usuarioService.eliminarUsuario(usuario.usuario_id)
      .pipe(
        finalize(() => {
      ...
      .subscribe({
        ...
        error: error => {
          this.error = error?.error?.message || 'No se pudo eliminar el usuario.';
          this.cdr.detectChanges();
        }
      });
  }
  ...
  esMiUsuario(usuario: Usuario): boolean {
    return usuario.usuario_id === this.authService.currentUsuario()?.usuario_id;
  }
}
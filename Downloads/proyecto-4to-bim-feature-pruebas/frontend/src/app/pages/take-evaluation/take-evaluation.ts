...
export class TakeEvaluation implements OnInit {
  ...
  evaluacionIdActual: number = 1; 

  ngOnInit(): void {
    const idParam = this.route.snapshot.paramMap.get('id');
    
    if (idParam) {
      this.evaluacionIdActual = Number(idParam);
    }
  ...
  finalizarEvaluacion(): void {
    ...
    const payloadResultado = {
      candidato_id: this.candidateId,
      evaluacion_id: this.evaluacionIdActual,
      puntaje: this.puntajeObtenido,
      porcentaje: this.porcentajeObtenido,
      aprobado: this.aprobado,
      fecha: new Date().toLocaleDateString()
    };

    // Almacenamos el resultado de manera local para que el ranking lo lea sin errores
    const resultadosPrevios = JSON.parse(localStorage.getItem('evaluacion_resultados') || '[]');
    resultadosPrevios.push(payloadResultado);
    localStorage.setItem('evaluacion_resultados', JSON.stringify(resultadosPrevios));
    
    console.log('Resultado guardado localmente de forma exitosa:', payloadResultado);
